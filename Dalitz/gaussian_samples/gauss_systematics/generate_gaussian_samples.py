...
if(total==9):
	...
	distance_of_centers_std = float(sys.argv[7])
	name = str(sys.argv[8])
else:	
...
rand_shift = gauss(0.0,distance_of_centers_std)
distance_oneD=(distance_of_centers+rand_shift)/np.sqrt(no_dim)
if verbose:
	print(rand_shift)
	print(distance_oneD)
...
np.savetxt("gauss_data/data_high{0}Dgauss_".format(int(no_dim))+str(int(no_points))+"_"+str(original_mean)+"_"+str(original_std)+"_"+str(distance_of_centers)+"_" + name + "_" +str(distance_of_centers_std)+"_"+str(int(label_no))+  ".txt",full_cords)
